...
namespace MultiplayerCloseAllNetworkPeers
{
    void GemSystemComponent::Reflect(ReflectContext* context)
    {
        if (auto sc = azrtti_cast<SerializeContext*>(context))
        {
            ...
            {
                ec->Class<GemSystemComponent>("Close All Network Peers", "[Closes all connected network peers]")
    ...
    {
        AZ_TracePrintf("", "OnGameEntitiesStarted");
        m_isMapLoaded = true;

        if (m_isHost && m_workerEntity == nullptr)
        {
            AZ_TracePrintf("", "OnGameEntitiesStarted - Entity");
            CreateWorkerEntity();
        }
    }
    ...
    void GemSystemComponent::OnCrySystemInitialized(ISystem& system, const SSystemInitParams&)
    {
    ...
    void GemSystemComponent::CreateWorkerEntity()
    {
        ...
        if (m_workerEntity)
        {
            m_workerEntity->CreateComponent<AzFramework::NetBindingComponent>();
            m_workerEntity->CreateComponent<WorkerComponent>();
            m_workerEntity->Init();
            m_workerEntity->Activate();

            AZ::EntityBus::MultiHandler::BusConnect(m_workerEntity->GetId());
        }

        AZ_Assert(m_workerEntity != nullptr, "Could not create worker Entity.");
        AZ_Assert(m_workerEntity->GetState() == AZ::Entity::ES_ACTIVE, "Worker entity could not be activated.");
    }
}
